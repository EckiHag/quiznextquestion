...
<div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
  {frage.options.map((option, index) => (
    <button
      key={index}
      id={`answer${index + 1}`}
      className={`bg-[#008cba] text-white border-none py-1 sm:py-2 px-2 sm:px-4 rounded-lg text-xs sm:text-xl h-14 sm:h-28 transition-all ease-in-out duration-300 hover:bg-[#005a6e]`}
