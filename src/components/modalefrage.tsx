...
export default function ModaleFrage({ frage }: ModaleFrageProps) {
  ...
  if (frage.question === "Joker") {
    ...
    return (
      <div className="flex flex-col items-center bg-[#005a6e] h-screen">
        <div className="h-[70vh] w-[90vw] text-white bg-[#005a6e] px-24">
        <div className="text-center text-2xl sm:text-4xl mt-2">Frage: {frage.question}</div>
<div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
  {frage.options.map((option, index) => (
    <button
      key={index}
      id={`answer${index + 1}`}
      className={`bg-[#008cba] text-white border-none py-1 sm:py-2 px-2 sm:px-4 rounded-lg text-sm sm:text-xl h-14 sm:h-28 transition-all ease-in-out duration-300 hover:bg-[#005a6e]`}
